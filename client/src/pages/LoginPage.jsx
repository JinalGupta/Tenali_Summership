...
export default function LoginPage() {
  ...
  const { email, step, loading, error, success, user,
    setEmail, setStep, setError,
    sendOTP, verifyOTP, fetchMe } = useAuthStore()
    ...
    <div className="min-h-screen bg-cream flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-navy flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/60 mt-6 text-lg leading-relaxed">Continue your journey through mathematical theorems.</p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-2 mb-8">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold bg-teal text-white">1</div>
            ...
            {step === 'email' ? (
              <motion.div key="email" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                ...
                <button onClick={handleSendOTP} disabled={loading}
                  ...
                  {loading ? <span className="animate-pulse">Sending code…</span> : 'Send login code'}
                </button>
